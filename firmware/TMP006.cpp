...
boolean TMP006::begin(uint8_t samplerate) {
    Wire.begin();
...
double TMP006::readDieTempC(void) {
...
double TMP006::readObjTempC(void) {
    double Tdie = readRawDieTemperature();
...
int16_t TMP006::readRawDieTemperature(void) {
...
int16_t TMP006::readRawVoltage(void) {
    int16_t raw;
...
uint16_t TMP006::read16(uint8_t a) {
    uint16_t ret;
...
void TMP006::write16(uint8_t a, uint16_t d) {
    Wire.beginTransmission(_addr); // start transmission to device
    Wire.write(a); // sends register address to read from
